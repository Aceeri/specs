...
mod test {
    ...
    impl Component for FlaggedCvec {
        ...
        type Metadata = metadata![ Flagged ];
    }
    ...
    fn flagged() {
        ...
        for i in 0..15 {
            ...
            assert!(s1.find::<Flagged, _>().flagged(Entity::new(i, Generation::new(1))));

            if i % 2 == 0 {
                s2.insert(Entity::new(i, Generation::new(1)), i.into());
                assert!(s2.find::<Flagged, _>().flagged(Entity::new(i, Generation::new(1))));
            }
        }

        s1.find_mut::<Flagged, _>().clear_flags();

        // Cleared flags
        for (entity, _) in (entities, &s1.check()).join() {
            assert!(!s1.find::<Flagged, _>().flagged(&entity));
        }
        ...
        for (entity, _) in (entities, &s1.check()).join() {
            ...
            if s1.find::<Flagged, _>().flagged(&entity) {
        ...
        for (entity, _, _) in (&*w.entities(), &s1, s1.find::<Flagged, _>()).join() {
